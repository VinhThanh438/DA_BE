...
import { CommonApproveStatus, DEFAULT_EXCLUDED_FIELDS, OrderStatus } from '@config/app.constant';
import { Prisma } from '@prisma/client';

export abstract class BaseService<T, S, W, R extends BaseRepo<T, any, W> = BaseRepo<T, any, W>> {
    protected db = DatabaseAdapter.getInstance().getClient();
    ...
    protected async validateForeignKeys<T extends { [key: string]: any }>(
        ...
        if (!data || Object.keys(foreignKeysAndRepos).length === 0 || !isCheck) return;
    ...
    async validateStatusApprove(
        id: number,
        repo: any = this.repo,
        status: CommonApproveStatus = CommonApproveStatus.PENDING,
    ) {
        const entity = await repo.findOne({ id, status } as any);
    ...
    public async canEdit(id: number, entityName: string, isAdmin: boolean, repo: any = this.repo) {
    ...
    protected autoMapConnection(items: any[], additionalFields?: Record<string, any>, skipFields?: string[]): any[] {
    ...
    protected canCheckForeignKeys(data: any[], currentData: any, keys: string[]): boolean {
    ...
    protected async findAndCheckExist<T extends { id?: number; [key: string]: any }>(
        item: T,
        ...
        if (whereCondition.OR.length === 0) return null;

        const existingItem: any = await this.repo.findOne(whereCondition, false, tx);

        if (existingItem) {
            // Find which field is duplicated
            const duplicatedField = fieldsToCheck.find(
                (field) => existingItem[field] === item[field] && item[field] != null,
            );

            throw new APIError({
                message: `common.status.${StatusCode.BAD_REQUEST}`,
                status: ErrorCode.BAD_REQUEST,
                errors: [`${duplicatedField}.${ErrorKey.EXISTED}`],
            });
        }
