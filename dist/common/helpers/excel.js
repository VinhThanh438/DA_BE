...
exports.readExcelData = exports.getMaxDepth = void 0;
exports.renderExcelHeader = renderExcelHeader;
...
const getMaxDepth = (headers, currentDepth = 0) => {
    ...
    for (const header of headers) {
        if (header.children && header.children.length > 0) {
            const childDepth = (0, exports.getMaxDepth)(header.children, currentDepth + 1);
 ...
 */
function renderExcelHeader(worksheet, headerData, startRow, startCol, dataCustomize, dataRender, dataFooter, template, numberRowTemplate) {
    try {
        if (dataCustomize) {
        ...
        const maxDepth = (0, exports.getMaxDepth)(headerData) + 1; // +1 vì tính cả level đầu tiên
        // Hàm helper để đếm tổng số cột leaf (các cột không có children)
        const countLeafColumns = (headers) => {
            let count = 0;
            for (const header of headers) {
                if (!header.children || header.children.length === 0) {
                    count++;
        ...
        const renderHeaders = (headers, rowStart, colStart, currentDepth) => {
            ...
            headers.forEach((header) => {
                ...
                if (header.children && header.children.length > 0) {
                    ...
                    if (currentDepth < maxDepth - 1) {
                        worksheet.mergeCells(rowStart, currentCol, rowStart + (maxDepth - currentDepth - 1), currentCol);
                    }
            ...
            for (let i = 1; i <= numberRowTemplate; i++) {
                ...
                for (let j = 1; j <= leafColumns.length; j++) {
                    ...
                    if (leafColumns[j - 1].dataValidation) {
                        cell.dataValidation = leafColumns[j - 1].dataValidation;
                    ...
                    if (leafColumns[j - 1].dataType && leafColumns[j - 1].dataType === 'list') {
                        if (leafColumns[j - 1].dataTypeOptions && leafColumns[j - 1].dataTypeOptions.length > 0) {
                            cell.dataValidation = {
                                ...
                                allowBlank: true,
                                formulae: [
                                    `"${leafColumns[j - 1].dataTypeOptions.map((item) => item).join(',')}"`,
                                ],
                            };
                        }
                    ...
                    if (leafColumns[j - 1].formula !== null && leafColumns[j - 1].formula !== undefined) {
                        const formula = leafColumns[j - 1].formula.replace(/\{i}/g, (i + startRow).toString());
                        cell.value = { formula };
                    }
                }
            }
            ...
            dataRender.forEach((data, index) => {
                ...
                for (let i = 1; i <= leafColumns.length; i++) {
                    ...
                    if (leafColumns[i - 1].dataValidation) {
                        cell.dataValidation = leafColumns[i - 1].dataValidation;
                        if (cell.dataValidation.type === 'date') {
                            cell.numFmt = 'dd/mm/yyyy';
                        }
                        else {
                    ...
                    if (leafColumns[i - 1].dataType && leafColumns[i - 1].dataType === 'list') {
                        if (leafColumns[i - 1].dataTypeOptions && leafColumns[i - 1].dataTypeOptions.length > 0) {
                            cell.dataValidation = {
                                ...
                                allowBlank: true,
                                formulae: [
                                    `"${leafColumns[i - 1].dataTypeOptions.map((item) => item).join(',')}"`,
                                ],
                            };
                        }
                    ...
                    if (leafColumns[i - 1].formula !== null && leafColumns[i - 1].formula !== undefined) {
                        const formula = leafColumns[i - 1].formula.replace(/\{i}/g, (index + startRow + maxDepth).toString());
                        cell.value = { formula };
                    }
                }
            });
        ...
        logger_1.default.error('Error rendering Excel header:', error);
...
function calculateLeaves(node) {
    if (node.children.length === 0)
        return 1;
    return node.children.reduce((sum, child) => sum + calculateLeaves(child), 0);
}
...
function buildHeaders(headerDefinitions) {
    const maxDepth = Math.max(...headerDefinitions.map(getNodeDepth));
    let totalColumns = headerDefinitions.reduce((sum, node) => sum + calculateLeaves(node), 0);
    const rows = Array.from({ length: maxDepth }, () => Array(totalColumns).fill(null));
    const columnsWidth = {};
    function processNode(node, depth, startCol) {
    ...
    let currentCol = 0;
    headerDefinitions.forEach((node) => {
...
function generateExcel(headers, worksheet, startRow) {
    const { rows, columnsWidth } = buildHeaders(headers);
    // Thêm các hàng vào worksheet
    rows.forEach((row, rowIdx) => {
        const rowValues = row.map((cell) => (cell === null || cell === void 0 ? void 0 : cell.header) || '');
        ...
        row.forEach((cell, colIdx) => {
            if (!cell || cell.colSpan === undefined)
                return; // Bỏ qua ô null hoặc không có colSpan
            const excelCell = excelRow.getCell(colIdx + 1);
            excelCell.style = cell.style;
            // Merge cells nếu có colspan
            if (cell.colSpan > 1) {
                worksheet.mergeCells(rowIdx, colIdx + 1, rowIdx, colIdx + cell.colSpan);
            }
        });
    });
    // Đặt độ rộng cột
    Object.entries(columnsWidth).forEach(([colIdx, width]) => {
        worksheet.getColumn(parseInt(colIdx) + 1).width = width;
    });
...
const readExcelData = (worksheet, headerData, startRow, startCol, dataCustomize) => {
    try {
        const data = [];
        ...
        for (let i = 0; i < headerData.length; i++) {
            if (worksheet.getCell(startRow, i + startCol).value !== headerData[i].header) {
                throw new Error(`Tiêu đề ở cột ${i}: '${worksheet.getCell(startRow, i + startCol).value}' không khớp với tệp mẫu, giá trị đúng là: '${headerData[i].header}', hãy kiểm tra lại tệp mẫu`);
            }
        }
            ...
            for (let j = startCol; j <= maxCol; j++) {
                ...
                if (cell.value !== null && typeof cell.value === 'object') {
                    if (cell.value && typeof cell.value === 'object' && 'result' in cell.value) {
                        rowData[headers[j - startCol]] = cell.value.result;
                    }
            ...
            dataCustomize.forEach((item) => {
                ...
                if (cell.value !== null) {
                    if (typeof cell.value === 'object' && 'result' in cell.value) {
                        dataCustomizeCell[item.key] = cell.value.result;
                    }
                    else {
                        logger_1.default.info(dataCustomizeCell[item.key], cell.value);
